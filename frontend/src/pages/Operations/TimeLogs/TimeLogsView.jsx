...
const TimeLogs = ({ employeeScope = false, leadScope = false }) => {
 ...
 doc.setFontSize(18);
 doc.text(
 `Time Logs Report - ${viewMode === "daily" ? "Daily Summary" : "Detailed Logs"}`,
 14,
 20,
 );
 ...
 doc.save(
 `time-logs-${viewMode}-${new Date().toISOString().split("T")[0]}.pdf`,
 );
 setExportDropdownOpen(false);
 };
 ...
 wb,
 ws,
 viewMode === "daily" ? "Daily Summary" : "Detailed Logs",
 );

 // Save file
 XLSX.writeFile(
 wb,
 `time-logs-${viewMode}-${new Date().toISOString().split("T")[0]}.xlsx`,
 );
 setExportDropdownOpen(false);
 };
 ...
 link.download = `time-logs-${viewMode}-${new Date().toISOString().split("T")[0]}.csv`;
 link.click();
 ...
 className="bg-white rounded-xl shadow-xl max-w-2xl w-full flex flex-col max-h-[90vh]"
 >
 ...
 <h4 className="text-sm font-medium text-gray-900 mb-4 border-l-4 border-black pl-3">
