...
const ContractCreate = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await apiClient.post('/operation/contracts/', data, {
                headers: { 'Content-Type': 'multipart/form-data' }
            });
            navigate('/operations/contracts');
        } catch (err) {
            console.error('Error creating contract:', err);
            alert('Failed to create contract. Please check all required fields.');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="p-6 bg-gray-50 min-h-screen">
            <div className="max-w-6xl mx-auto">
                <div className="flex items-center gap-4 mb-6">
                    ...
                    <h1 className="text-2xl font-bold text-gray-800">Add Contract</h1>
                </div>

                <form onSubmit={handleSubmit} className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                    <div className="bg-gray-50 px-8 py-4 border-b border-gray-100">
                        <h2 className="text-sm font-bold uppercase tracking-wider text-gray-500">Add Contract Details</h2>
                    </div>
                    ...
                    <div className="px-8 py-6 bg-gray-50 border-t border-gray-100 flex gap-4">
                        ...
                        >
                            {loading ? <><MdRefresh className="animate-spin" /> Saving...</> : <><MdSave /> Save</>}
                        </button>
                        <button
                            type="button"
                            onClick={() => setFormData({ ...formData, subject: '' })} // Reset logic simplified
                            className="flex items-center gap-2 bg-gray-700 text-white px-8 py-3 rounded-xl font-bold hover:bg-gray-800 transition-all shadow-md"
                        >
                            <MdRefresh /> Reset
                        </button>
                    </div>
