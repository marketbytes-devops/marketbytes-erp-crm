                            ...
                            <div className="flex justify-between items-start mb-6">
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        <span className={`px-4 py-1.5 rounded-full text-xs font-bold border ${getStatusColor(project.status)} uppercase tracking-wider`}>
                                            {project.status?.name || "No Status"}
                                        </span>
                                        <span className="px-4 py-1.5 rounded-full text-xs font-bold bg-purple-50 text-purple-700 border border-purple-100 uppercase tracking-wider">
                                            {project.category?.name || "General"}
                                        </span>
                                        {project.department && (
                                            <span className="px-4 py-1.5 rounded-full text-xs font-bold bg-amber-50 text-amber-700 border border-amber-100 uppercase tracking-wider">
                                                {project.department.name}
                                            </span>
                                        )}
                                        {project.stage && (
                                            <span className="px-4 py-1.5 rounded-full text-xs font-bold bg-indigo-50 text-indigo-700 border border-indigo-100 uppercase tracking-wider">
                                                {project.stage?.name}
                                            </span>
                                        )}
                                    </div>
                        ...
                        >
                            <div className="p-8 border-b border-gray-100 flex justify-between items-center">
                                ...
                                <Link
                                    ...
                                    state={{ projectId: id }}
                                    className="px-4 py-2 bg-blue-50 text-blue-700 rounded-xl hover:bg-blue-100 transition text-sm font-bold flex items-center gap-1"
                                >
                                            ...
                                            tasks.map((task) => (
                                                <tr key={task.id} className="hover:bg-gray-50 transition group">
                                                    <td className="px-8 py-5">
                                                        <Link to={`/operations/tasks/edit/${task.id}`} className="font-semibold text-gray-900 hover:text-blue-600 transition">
                                                            {task.name}
                                                        </Link>
                                                    </td>
                                                    <td className="px-8 py-5">
                                                        <div className="flex -space-x-2">
                                                            {task.assignees?.slice(0, 3).map((a, i) => (
                                                                <div key={i} className="w-8 h-8 rounded-full bg-gray-200 border-2 border-white flex items-center justify-center text-[10px] font-bold text-gray-600 shadow-sm" title={a.name || a.username}>
                                                                    {(a.name || a.username || "?")[0].toUpperCase()}
                                                                </div>
                                                            ))}
                                                            {task.assignees?.length > 3 && (
                                                                <div className="w-8 h-8 rounded-full bg-gray-600 border-2 border-white flex items-center justify-center text-[10px] font-bold text-white shadow-sm">
                                                                    +{task.assignees.length - 3}
                                                                </div>
                                                            )}
                                                        </div>
                            ...
                            {project.client ? (
                                <div className="space-y-4">
                                    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-2xl">
                                        <div className="w-12 h-12 rounded-2xl bg-black flex items-center justify-center text-white text-xl font-bold shadow-lg">
                                            {project.client.name[0].toUpperCase()}
                                        </div>
                                        <div>
                                            <p className="font-bold text-gray-900 leading-tight">{project.client.name}</p>
                                    ...
                                    <div className="grid grid-cols-1 gap-3">
                                        {project.client.phone && (
                                            <div className="p-3 border border-gray-100 rounded-2xl">
                                                ...
                                                <p className="text-sm font-semibold text-gray-900">{project.client.phone}</p>
                                            </div>
                                        )}
                                        {project.client.address && (
                                            <div className="p-3 border border-gray-100 rounded-2xl">
                                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Address</p>
                                                <p className="text-sm font-semibold text-gray-900">{project.client.address}</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                {project.members?.length > 0 ? (
                                    project.members.map((member) => (
                                        <div key={member.id} className="flex items-center gap-3 p-2 hover:bg-gray-50 rounded-2xl transition group">
                                            <div className="w-10 h-10 rounded-xl bg-linear-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm shadow-md transition group-hover:scale-110">
                                                {(member.name || member.email)[0].toUpperCase()}
                                            </div>
                                            <div className="overflow-hidden">
                                                <p className="text-sm font-bold text-gray-900 truncate">{member.name || "Unnamed User"}</p>
                                    ...
                                    project.project_files.map((file) => (
                                        ...
                                        >
                                            <div className="flex items-center gap-3 overflow-hidden">
                                                ...
                                                <p className="text-sm font-semibold text-gray-700 truncate">{file.original_name}</p>
                                            </div>
                                            <span className="text-[10px] font-bold text-blue-600 uppercase group-hover:underline whitespace-nowrap">View</span>
                                        </a>
                                    ))
                                ) : (
