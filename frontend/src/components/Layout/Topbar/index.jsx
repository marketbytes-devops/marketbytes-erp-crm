...
const Topbar = ({
  ...
  const [displayDesignation, setDisplayDesignation] = useState("Guest");
  ...
  useEffect(() => {
    ...
    if (user?.designation?.name) {
      setDisplayDesignation(user.designation.name);
    }
    if (user?.image) {
    ...
    const fetchProfile = async () => {
      try {
        ...
        // Show designation name if available, otherwise fallback to "Guest"
        if (data.designation && data.designation.name) {
          setDisplayDesignation(data.designation.name);
        } else {
          setDisplayDesignation("Guest");
        }
        if (data.image) {
  ...
  const profileTrigger = (
    ...
    >
      <div className="text-right leading-tight hidden sm:block">
        <p className="whitespace-nowrap text-xs font-medium text-gray-900 group-hover:text-black transition-colors">
          {displayName}
        </p>
        {displayDesignation && (
          <p className="whitespace-nowrap text-[10px] text-gray-500 group-hover:text-gray-600 transition-colors">
            {displayDesignation}
          </p>
        )}
      </div>
  ...
  return (
    <div
      className="flex items-center justify-between px-4 sm:px-5 py-2.5 text-sm bg-white shadow-lg backdrop-blur-sm border-b border-gray-100 rounded-b-xl mx-4 sm:mx-6 z-50"
    >
