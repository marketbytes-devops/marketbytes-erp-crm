...
from .models import Company, Client, Lead, LeadSource, LeadCategory, LeadTeam,RfpTemplate,ProposalTemplate
from .serializers import (
    CompanySerializer, ClientSerializer, LeadSerializer,
    LeadSourceSerializer, LeadCategorySerializer, LeadTeamSerializer,RfpTemplateSerializer,ProposalTemplateSerializer
)
...
class LeadViewSet(viewsets.ModelViewSet):
    queryset = Lead.objects.select_related('company', 'client', 'lead_agent__user', 'lead_source', 'lead_category', 'lead_team').all()
    serializer_class = LeadSerializer
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    search_fields = ['client__name', 'client__email', 'company__name', 'notes']
    filterset_fields = ['status', 'lead_agent', 'lead_source', 'lead_category', 'lead_team']
